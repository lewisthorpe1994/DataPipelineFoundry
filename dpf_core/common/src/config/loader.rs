...
pub fn read_config(project_config_path: Option<PathBuf>) -> Result<FoundryConfig, ConfigError> {
    ...
    let connections_path = resolve_path(
        &config_root,
        Path::new(&proj_config.connection_profile.path),
    );
    ...
    for (profile, value) in raw_connections.into_iter() {
        let profile_connections = parse_connection_profile(value).map_err(|err| {
            ConfigError::parse_error(format!(
                "Failed to parse connection profile '{profile}': {err}"
            ))
        })?;
    ...
    let wh_sources = {
        let config = resolved_sources
            .get(&SourceType::Warehouse)
            .ok_or_else(|| ConfigError::parse_error("Warehouse source not defined"))?;
        ...
        match config {
            Some(config) => {
                let definitions_config = match &config.definitions {
                    Some(def) => load_config::<KafkaConnectorConfig>(def)?,
                    None => HashMap::new(),
                };
                (
                    load_config::<KafkaSourceConfig>(&config.specifications)?,
                    definitions_config,
                )
            }
    ...
    let src_db_sources = {
        let config = resolved_sources
            .get(&SourceType::SourceDB)
            .ok_or_else(|| ConfigError::parse_error("No source databases defined"))?;
        load_config::<DbConfig>(&config.specifications)?
    };
