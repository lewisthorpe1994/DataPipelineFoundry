...
impl From<Vec<SourcePaths>> for WarehouseSourceConfigs {
    fn from(value: Vec<SourcePaths>) -> Self {
        ...
        value.into_iter().filter(|t| t.kind == SourceType::Warehouse).for_each(|p| {
            let c: WarehouseSourceConfigs =
                load_config::<WarehouseSourceConfig, WarehouseSourceFileConfigs, WarehouseSourceConfigs>((&p.path).as_ref())
                    .expect("s");
            configs.extend(c);
        });
